...
import {
  BitacoraUsuarioService,
  BitacoraUsuarioDto,
  BitacoraUsuarioPage
} from '../service/bitacora-usuario.service';
...
export class UsuarioMonitoreoComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('bitacoraList', { static: false })
  bitacoraListRef?: ElementRef<HTMLDivElement>;
  ...
  loadBitacora(userId: string): void {
    ...
    this.bitacoraUsuarioService
      .searchBitacoraUsuario(userId, this.page, this.size, fechaParam)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
  ...
  formatValorAntes(valorAntes: string, valorDespues: string): SafeHtml {
    ...
    if (!valorDespues) {
      ...
      return this.sanitizer.bypassSecurityTrustHtml(
        this.formatJsonHighlight(valorAntes, 'removed')
      );
    }
    ...
    try {
      ...
      return this.sanitizer.bypassSecurityTrustHtml(
        this.formatObjectWithRemovals(antesObj, despuesObj)
      );
    } catch (error) {
      // Si no son JSON válidos, comparar como strings
      return this.sanitizer.bypassSecurityTrustHtml(
        this.highlightStringDiff(valorAntes, valorDespues, 'removed')
      );
    }
  }
  ...
  formatValorDespues(valorAntes: string, valorDespues: string): SafeHtml {
    ...
    if (!valorAntes) {
      ...
      return this.sanitizer.bypassSecurityTrustHtml(
        this.formatJsonHighlight(valorDespues, 'added')
      );
    }
    ...
    try {
      ...
      return this.sanitizer.bypassSecurityTrustHtml(
        this.formatObjectWithAdditions(antesObj, despuesObj)
      );
    } catch (error) {
      // Si no son JSON válidos, comparar como strings
      return this.sanitizer.bypassSecurityTrustHtml(
        this.highlightStringDiff(valorAntes, valorDespues, 'added')
      );
    }
  }

  private formatJsonHighlight(
    jsonString: string,
    type: 'added' | 'removed'
  ): string {
    try {
      ...
      const className =
        type === 'added' ? 'diff-added-line' : 'diff-removed-line';
  ...
  private formatObjectWithRemovals(antes: any, despues: any): string {
    ...
    removedKeys.forEach((key) => {
      ...
      const valorSimilarEnDespues = this.findSimilarValue(
        valueAntes,
        Object.values(despues || {})
      );

      if (valorSimilarEnDespues) {
        // Hay un valor similar, resaltar solo las diferencias
        const highlighted = this.highlightStringDiff(
          valueAntes,
          valorSimilarEnDespues,
          'removed'
        );
        ...
        const style =
          'background-color:rgba(244,67,54,0.5);color:#c62828;padding:3px 6px;border-radius:4px;font-weight:bold;';
    ...
    commonKeys.forEach((key) => {
      ...
      if (JSON.stringify(antesValue) !== JSON.stringify(despuesValue)) {
        ...
        const antesStr =
          typeof antesValue === 'string'
            ? antesValue
            : JSON.stringify(antesValue).replace(/^"|"$/g, '');
        const despuesStr =
          typeof despuesValue === 'string'
            ? despuesValue
            : JSON.stringify(despuesValue).replace(/^"|"$/g, '');
        const highlighted = this.highlightStringDiff(
          antesStr,
          despuesStr,
          'removed'
  ...
  private formatObjectWithAdditions(antes: any, despues: any): string {
    ...
    addedKeys.forEach((key) => {
      ...
      const valorSimilarEnAntes = this.findSimilarValue(
        valueDespues,
        Object.values(antes || {})
      );
      ...
      if (valorSimilarEnAntes) {
        ...
        const style =
          'background-color:rgba(76,175,80,0.5);color:#2e7d32;padding:3px 6px;border-radius:4px;font-weight:bold;';
  ...
  private highlightStringDiff(
    ...
    while (
      suffixStart < maxSuffixLen &&
      target[target.length - 1 - suffixStart] ===
        other[other.length - 1 - suffixStart]
    ) {
      suffixStart++;
    }
    ...
    if (
      diff.length === 0 ||
      (prefix.length === 0 && suffix.length === 0 && target !== other)
    ) {
      // Si no se detectó diferencia pero los strings son diferentes, mostrar todo
      const backgroundColor =
        type === 'removed' ? 'rgba(244,67,54,0.5)' : 'rgba(76,175,80,0.5)';
  ...
  private findSimilarValue(targetValue: string, values: any[]): string | null {
    ...
    for (const val of values) {
      ...
      if (minCommonLen >= 5) {
        ...
        let prefixLen = 0;
        while (
          prefixLen < minCommonLen &&
          targetStr[prefixLen] === valStr[prefixLen]
        ) {
          prefixLen++;
        }
        ...
        let suffixLen = 0;
        while (
          suffixLen < minCommonLen - prefixLen &&
          targetStr[targetStr.length - 1 - suffixLen] ===
            valStr[valStr.length - 1 - suffixLen]
        ) {
          suffixLen++;
        }
