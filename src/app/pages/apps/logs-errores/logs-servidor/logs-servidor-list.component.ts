...
@Component({
  selector: 'gm-logs-servidor-list',
  imports: [
    MatButtonModule,
    MatTooltipModule,
    MatTableModule,
  ...
  templateUrl: './logs-servidor-list.component.html',
  styleUrl: './logs-servidor-list.component.scss'
})
export class LogsServidorListComponent implements OnInit, OnDestroy {
  ...
  cellExtra(row: LogInfluxRow): string {
    return logRowExtraSummary(row);
  }

  isServerErrorLevel(row: LogInfluxRow): boolean {
    return logRowLevelNormalized(row) === 'ERROR';
  }

  isServerWarnLevel(row: LogInfluxRow): boolean {
    const u = logRowLevelNormalized(row);
  ...
  async exportarExcel(): Promise<void> {
    ...
    try {
      const rows = this.dataSource.map((row) => ({
        ...
        mensaje: this.cellMessage(row),
        otrosCampos: this.cellExtra(row)
      }));
      await downloadLogsAsXlsx(rows, {
        filenameBase: 'logs-servidor',
        sheetName: 'Logs servidor'
      });
    } catch (err) {
      console.error(err);
  ...
  exportarLog(): void {
    ...
    try {
      ...
      const doc = buildLogsPlainTextDocument('Logs servidor', rows);
      downloadPlainLogFile(doc, 'logs-servidor');
    } catch (err) {
      console.error(err);
